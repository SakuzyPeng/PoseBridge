...
measure_decode = run_path(str(root / 'scripts/measure_osc.py'))['decode']
...
for (fmt, expected), synthetic in itertools.product([
    ...
    source_id = 'integration-耳机' if synthetic else 'integration'
    ...
    try:
        command = [str(binary), 'simulate', '--source-id', source_id, '--yaw', '30', '--pitch', '20', '--roll', '10',
        ...
        run = subprocess.run(command, capture_output=True, text=True, encoding='utf-8', timeout=12)
    ...
    for data in packets:
        ...
        assert version == 3 and source == source_id and min(instance, session, sequence, tx, reference, revision) > 0
        ...
        assert measured == tuple(values[13:])
        pose_count += 1
