...
设备操作必须在姿态采集停止后执行，忙时返回 Busy；完成后由调用方显式重新启动。跨进程 CLI 也须先停止占用设备的 bridge。
0.6 的[独立磁场会话](magnetic.md)允许边读磁场边发送 mag_start／mag_stop／save；
使用 `posebridge magnetic monitor` 只读观察，或 `posebridge magnetic calibrate` 执行定时校准。
