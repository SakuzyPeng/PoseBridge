...
[验证记录](docs/validation.md)。MacinRender OSC 接收端尚需单独实现；成功发送 OSC 不等于音频软件已消费。
...
这些消息是明确的 PoseBridge 姿态约定，不是任何支持 OSC 的软件都能直接识别；详见[协议与坐标](docs/protocol.md)。
...
C ABI 以不透明 `PbContext` 和最新快照轮询工作，详见 [C ABI 文档](docs/c-api.md)及
